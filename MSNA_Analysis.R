...
df_hh <- read.csv("input/MSNA_HH_Analysed_data.csv", stringsAsFactors = F, encoding = "UTF-8")
names(df_hh) <- tolower(names(df_hh))
df_hh$mssc_2_source_rev_1 <- gsub("[^ -~]", "", response$mssc_2_source_rev_1)
df_hh$mssc_2_source_rev_2 <- gsub("[^ -~]", "", response$mssc_2_source_rev_2)
df_hh$mssc_2_source_rev_3 <- gsub("[^ -~]", "", response$mssc_2_source_rev_3)
df_hh$aap_1_source_confiance <- gsub("[^ -~]", "", response$aap_1_source_confiance)
...
df_hh <- df_hh %>%
  mutate(protect_9_auMoinsUnVul = if_else(sum_ig_7_gr_vulnerable.aucune == 0, "non", "oui"),
         ...
         score_casserole = case_when(
           ...
           nb_casseroles >= 4 & nb_casseroles <5 ~ 2
         ),
  ...
  mutate(
         score_nfi = (score_bidons+ score_seau+ score_moust+ score_casserole+ score_couchage+ score_draps)/6,
...
df_hh <- df_hh %>%
  ...
  mutate(
    ...
    lcs_total = if_else(lcs_urgence ==1, 4, if_else(lcs_crise == 1, 3, if_else( lcs_stress ==1, 2, 1)))
  )%>%
  mutate(fcs_score2 = secal_1_cereales*2 + secal_1_legumineuse*3 + secal_1_lait*4 + secal_1_viande*4 + secal_1_legumes*1 + secal_1_fruits*1 + secal_1_huile*0.5 + secal_1_sucre*0.5,
         ...
         fcs_score_borderline = if_else( fcs_score2 > 21 & fcs_score <= 35, 1,0),
  ...
  mutate(source_eau_boisson = if_else((wash_1_source_boisson.autre + wash_1_source_boisson.nsp) > 0, NA_character_,
         ...
         enfants_ecole_6mois = (sum_educ_3_presence_18_19.filles_7_12.12m + sum_educ_3_presence_18_19.filles_7_12.6m_12m + sum_educ_3_presence_18_19.filles_13_18.12m+ sum_educ_3_presence_18_19.filles_13_18.6m_12m+
                                sum_educ_3_presence_18_19.garcons_7_12.12m + sum_educ_3_presence_18_19.garcons_7_12.6m_12m + sum_educ_3_presence_18_19.garcons_13_18.12m+ sum_educ_3_presence_18_19.garcons_13_18.6m_12m)/(sum_educ_2_inscrit_18_19_non+sum_educ_2_inscrit_18_19_non),
